...
本次实验中 BPE 的处理流程如下：
\begin{enumerate}
    \item \textbf{预分词（Pre-tokenization）}：使用 \texttt{Sequence([Whitespace(), Punctuation()])}。先按空白切分，再将标点独立，避免跨词合并（例如避免将 \texttt{"hello world"} 误合并为 \texttt{"ellowo"}），提升 Token 的语义可读性。
    \item \textbf{初始化词表}：将语料拆解为最基础的字符（或字节）单元，构成初始词表。
    \item \textbf{统计与合并（Train/Merge）}：统计当前语料中所有相邻 Token 对的频次，选择最高频的一对（如 \texttt{t} 与 \texttt{h}），合并为新 Token（如 \texttt{th}），并在语料中完成对应替换。
