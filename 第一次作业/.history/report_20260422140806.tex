...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.3\textwidth} p{0.62\textwidth}}
        ...
        \midrule
        分词模型 & BPE \\
        词表大小（vocab\_size） & 6400 与 16384（分别训练两套分词器） \\
        最小词频（min\_frequency） & 2 \\
        特殊符号（special\_tokens） & \texttt{<|endoftext|>}、\texttt{<|im\_start|>}、\texttt{<|im\_end|>} \\
        未知词标记（unk\_token） & \texttt{<|endoftext|>} \\
        预分词策略（pre\_tokenizer） & 先按空白切分（Whitespace），再拆分标点（Punctuation） \\
        训练语料文件 & \texttt{mixed\_train.txt} \\
        \bottomrule
    \end{tabular}
\end{table}
